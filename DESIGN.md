...
Use 6–8px section gaps and a 60px place tile, reduced to 54px with no extra gap between padded plan rows on short phones. Art remains separate from the smaller Phosphor status marks. Put back joins the bottom controls when a placed piece is selected, while selection announcements remain available to screen readers. Completion uses two compact columns with Share above the countdown and matching Reset/Feedback utilities below. The Tutorial primary action spans the result group vertically so it is centered when there is no countdown. Preserve scrollable dialogs and readable recovery messages.
