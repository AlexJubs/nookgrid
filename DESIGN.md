...
Use the shared Grove palette, system typography and quiet controls on web and iPhone. The home screen makes the daily streak prominent, with a seven-day strip and one Play or Continue action. Pair the streak count with the same decorative Phosphor regular lightning icon on Home, All puzzles and completion. Size the Home lightning to three quarters of its streak number, scaling together; use 16px beside compact streak labels; write 6 day streak without a hyphen, including shared result text. New and completed days start at home; an unfinished daily attempt resumes directly. Explicit dated links and Tutorial links go straight to their puzzle. Tutorial starts fresh on entry or reload, without saved level progress. Help, View solved puzzle and backgrounding preserve the current attempt and screen.

Calendar is the only puzzle-history destination. Keep All puzzles and the lightning streak label together in one header row, reserving room for Close. Allow wrapping only when enlarged text cannot fit. Use a surfaced button with the local calendar icon and All puzzles label on home and result, with the same destination in the menu. It opens one month at a time, with Previous month and Next month buttons bounded by the launch and current months. Open the viewed archive’s month, or the current month from home and Tutorial. Preserve the displayed month and keyboard focus when saved history or the UTC puzzle date changes. Announce the month heading politely. Underline the current puzzle and preserve its accessible description. Completed saves receive checks; dates that earned daily streak credit are highlighted separately. Show one compact legend below the calendar: Streak with the accent fill, Completed with the light surface fill, and Uncompleted with a transparent swatch. Match swatches to the calendar in each theme; omit the explanatory sentence above the month. Archive play does not earn daily streak credit. Future, prelaunch and missing puzzles cannot be opened. Calendar days, controls and available week dates keep 44px targets. On Home and completion, tapping a week date or completed check opens that puzzle with its saved progress. Future, prelaunch and missing dates stay unavailable. Preserve week-link keyboard focus when history updates; at week rollover, focus the new current date if the old date is no longer visible.
...
Keep the outcome, streak, week strip and supporting countdown together above one group of three full-width actions: Share result, View solved puzzle and All puzzles. Tutorial substitutes Play today's puzzle for Share result. Pair these completion labels with matching 20px Phosphor regular icons: Share, Eye, Calendar and Play for the Tutorial action. Use matching secondary buttons for the two navigation actions. Undo, Reset and Hint are only for unfinished puzzles. Feedback stays in Menu and Home in the header; do not duplicate them on the recap. Home and completion share the same action widths, 52px minimum heights, 12px gaps and bottom anchor within the safe viewport. Allow natural scrolling on short or enlarged-text screens. Archive completion shows the solved puzzle’s Monday-to-Sunday week with a dated week heading; Home and Tutorial use the current week. Availability still follows today’s UTC date. Make the countdown easy to read with 40px medium-weight tabular digits and a quiet Next puzzle in label above it. The menu uses four matching secondary buttons: All puzzles, Settings, Feedback and Privacy. All puzzles opens the existing calendar; there is no separate puzzle list. Dates open the selected puzzle directly and preserve current, completed and earned-streak descriptions. Home uses one primary daily action: Play or Continue today’s puzzle, changing to View today’s result when solved. All puzzles and Play tutorial remain matching secondary buttons underneath. All three visible actions pair text with 20px Phosphor icons: Play for starting or continuing, Grid Four for results, Calendar for all puzzles and Video for Tutorial. Tutorial remains available in How to play. Each entry starts fresh practice. Reuse saved completion history for dated puzzles so Reset and replay retain completion; Tutorial completion lasts only for the current visit.
