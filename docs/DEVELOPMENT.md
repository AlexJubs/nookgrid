...
Public top-level game links load gameplay when the screen's shorter dimension is at least 600 CSS pixels; smaller screens redirect to the fixed App Store listing before importing gameplay or analytics. Screen dimensions keep a rotated phone on the store route and a narrow laptop window playable. Embedded public pages retain the download link. Native bundles wait for the Capacitor bridge, and HTTP(S) loopback development servers always load the game. A public `?test=1` does not bypass phone routing. `tests/e2e/download.spec.mjs` intercepts reserved test origins and the store destination locally, with analytics disabled and external requests blocked. Shared puzzle dates are not transferred through App Store installation.
